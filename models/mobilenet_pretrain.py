...
#Pretrain on LungOpacity
TRAIN_LUNG_PATH_CSV = './data/COVID-19_Radiography_Dataset/Lung_Opacitytrain.xlsx'
VALID_LUNG_PATH_CSV = './data/COVID-19_Radiography_Dataset/Lung_Opacityvalid.xlsx'
LUNG_PATH = './data/COVID-19_Radiography_Dataset/Lung_Opacity'
...
class COVIDdata(Dataset):
    ...
    def __init__(self,
        ...
        if pos_path == TRAIN_POS_PATH_CSV:
            self.df = self.df.drop(self.df.index[[6500,9000]])
...
train_dataset = COVIDdata(pos_path=TRAIN_LUNG_PATH_CSV,
                              pos_dir=LUNG_PATH,
...
valid_dataset = COVIDdata(pos_path=VALID_LUNG_PATH_CSV,
                              pos_dir=LUNG_PATH,
...
best_mae, best_rmse, best_epoch, num_epochs = 999, 999, -2, 5
...
#same as CE loss, since softmax is output layer
        ...
        if not batch_idx % 50:
            ...
            print(s)
            with open("outputpre.txt", "a") as f:
                f.write('%s\n' % s)
